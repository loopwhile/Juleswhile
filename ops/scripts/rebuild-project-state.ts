...
async function githubRequest<T>(
  ...
  if (!response.ok) {
    fail(
      `GitHub API 요청 실패 HTTP ${response.status}: ${body.slice(0, 500)}`,
    );
  }
...
async function observeSessions(
  ...
  if (apply && errors.length > 0) {
    fail(
      "Jules Session 조회 오류가 있어 마지막 유효 Projection을 보존합니다.",
    );
  }
...
async function validateProjectState(
  ...
  const parsedGoalSchema =
    JSON.parse(goalSchema) as object;

  const parsedStateSchema =
    JSON.parse(stateSchema) as {
      $id?: string;
    };
  ...
  if (
    typeof parsedStateSchema.$id !==
      "string" ||
    parsedStateSchema.$id.trim() === ""
  ) {
    fail(
      `${PROJECT_STATE_SCHEMA}에 유효한 $id가 없습니다.`,
    );
  }

  const valid =
    ajv.validate(
      parsedStateSchema.$id,
      value,
    );
...
async function main(): Promise<void> {
  ...
  const input: ProjectionInput = {
    ...
    pullRequests,
    sessionsByName:
      sessionObservation.sessionsByName,
    runUrl:
  ...
  await validateProjectState(
    projection.projectState,
  );
  ...
  const response = {
    ...
    taskSummary:
      projection.projectState
        .taskSummary,
    runtime:
      projection.projectState.runtime,
    quotas:
      projection.projectState.quotas,
    drift: projection.drift,
    sessionLookupErrors:
      sessionObservation.errors,
    files: {
      projectState:
        options.projectStatePath,
      taskIndex:
  ...
  console.log(
    `Runtime State Projection 완료: changed=${projection.changed}`,
  );
}
...
main().catch((error: unknown) => {
  ...
  console.error(
    `Runtime State Projection 실패: ${message}`,
  );
